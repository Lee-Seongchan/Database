...
INSERT INTO `Users` VALUES ('user4', '이사부','1989-04-13', 'M', '010-2101-1979', 'leesabu@gamil.com', 2600, 1, '서울', '2022-01-10 10:50:12');
INSERT INTO `Users` VALUES ('user5', '장보고','1979-09-12', 'M', '010-2104-1966', 'jangbg@gmail.com', 9400, 4, '대전', '2022-01-10 10:50:12');
INSERT INTO `Users` VALUES ('user6', '선덕여왕','1966-07-28', 'M', '010-3101-1979', 'queen@naver.com', 1600, 2, '대전', '2022-01-10 10:50:12');
...
INSERT INTO `Points`(`userId`, `point`, `pointDesc`,`pointDate`) VALUES('user5', 3000, '이벤트 응모 3000적립','2022-01-10 10:50:12');
...
INSERT INTO `Points`(`userId`, `point`, `pointDesc`,`pointDate`) VALUES('user3', 1500, '이벤트 응모 1500적립','2022-01-10 10:50:12');
...
INSERT INTO `Sellers` VALUES (10006, '복실이웃짱', '051-301-1979', '고소용', '부산');
...
INSERT INTO `Sellers` VALUES (10010, '누리푸드', '051-170-1992', '강동원', '부산');
...
INSERT INTO `Orders` VALUES ('22010410001', 'user6', 792000, '부산시 남구 21-1', 2, '2022-01-10 10:50:12');
...
INSERT INTO `Orders` VALUES ('22010710010', 'user9', 94500, '광주시 충열로 11', 1, '2022-01-10 10:50:12');
...
INSERT INTO `Carts` (`userId`, `prodNo`, `cartProdCount`,`cartProdDate`)VALUES ('user2', 110101, 1, '2022-01-10 10:50:12');
...
#실습 1 모든 장바구니 내역에서 고객명, 상품명, 상품수량 조회하시오 단 상품수량 2건이상.
SELECT b.`userId`, `prodName`,`prodStock`
FROM `carts`    AS a 
...
# 문제2 모든 상품내역에서 상품번호, 상품 카테고리명, 상품명, 상품가격, 판매자 이름, 판매자 연락처를 조회하시오.
SELECT 
	`prodNo`,
	`cateName`,
	`prodName`,
...
#3 모든 고객의 아이디, 이름, 휴대폰, 현재포인트, 적립 포인트 총합을 조회하시오. 단 적립포인트 내역이 없으면 0으로 출력
SELECT 
	a.`userId`,
	`userName`,
	`userHp`,
...
#문제 4 모든 주문의 주문번호, 주문자 아이디, 고객명, 주문가격, 주문일자를 조회하시오
# 단 주문금액에 10만원 이상 큰금액순으로 조회 금액이 같으면 이름이 사전순으로 될 것 
...
FROM `orders`  	AS a 
JOIN `users`   	AS b ON a.`userId`= b.`userId`
JOIN `orderitems` AS c ON c.orderNo = a.orderNo
JOIN `products` 	AS d ON c.prodNo 	= c.prodNo
GROUP BY a.orderNo;


#문제 6. 모든 상품의 상품 번호, 상품명, 상품가격, 할인율, 할인된 가격을 조회하시오
SELECT 
	`prodNo`,
	`prodName`,
	`prodPrice`,
	`prodDiscount`,
	(prodprice - prodprice*(prodDiscount/100)) AS `할인된 가격`
FROM `products`  



#문제 7. 고소영이 판매하는 모둔 상품의 상품번호, 상품명, 상품가격, 재고수량 ,판매자이름을 조회 하시오
SELECT 
	a.`sellerNo`,
	`prodNo`,
	`prodName`,
...
WHERE `sellerManager` = '고소용';



#문제 8. 아직 상품을 판매하지 않은 판매자의 판매자 번호, 판매자 상호, 판매자 이름, 판매자 연락처를 조회하시오
...
#문제 9. 모든 주문상세내역 중 개별 상품 가격과 개수 그리고 할인율이 적용된 총합을 구하고 
# 주문별 총합을 구해서 주문 총합이 10만원 이상 그리고 큰 금액순으로 주문번호, 최종총합을 구하여라

SELECT 
	a.orderNo,
	SUM((itemPrice * itemCount) -((itemprice*itemcount)*(itemDiscount/100))) AS `최종총합`
FROM `orders`     AS a
JOIN `orderitems` AS b ON a.orderNo = b.orderNo
JOIN `products`  	AS c ON b.prodNo = c.prodNo
GROUP BY `orderNo`
HAVING `최종총합` >= 100000
order BY `최종총합`DESC, `orderNo` DESC;
...
# 문제10 장보고 고객이 주문했던 모든 상품명을 고객명, 상품명으로 조회하시오 상품명 중복안됨
SELECT 
	`userName`,
	GROUP_CONCAT(`prodName`)AS `상품명`
FROM `Users` 		AS a 
